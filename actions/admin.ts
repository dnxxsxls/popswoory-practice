...
import { deactivateMember, getMember, resetMemberPassword } from "@/lib/store";
...
export async function deleteMemberAccount(
  ...
  const me = await requireMember();
  if (me.role !== "admin") {
    return { ok: false, error: "관리자만 회원 계정을 삭제할 수 있어요." };
  }

  const memberId = memberIdSchema.safeParse(rawMemberId);
  if (!memberId.success) return { ok: false, error: "회원 정보가 올바르지 않아요." };
  if (memberId.data === me.memberId) {
    return { ok: false, error: "현재 로그인한 관리자 계정은 삭제할 수 없어요." };
  }

  const member = await getMember(memberId.data);
  if (!member?.isActive) return { ok: false, error: "회원을 찾을 수 없어요." };

  const deleted = await deactivateMember(member.id);
  if (!deleted) {
    return { ok: false, error: "마지막 관리자 계정은 삭제할 수 없어요." };
  }
